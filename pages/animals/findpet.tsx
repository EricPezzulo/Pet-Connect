...
const findpet = ({
  listOfCatBreeds: cats,
  listOfDogBreeds: dogs,
}: breedProps) => {
  ...
  const displayDogResults =
    displayDogFilteredResults?.length === 0 || !displayDogFilteredResults ? (
  ...
  const displayCatResults =
    displayCatFilteredResults?.length === 0 || !displayCatFilteredResults ? (
      <div className="px-5 py-3 flex items-center">
        {" "}
        <p className="text-xl font-light">No Results Found&nbsp;&#128533;</p>
      </div>
      ...
      <>
        {displayCatFilteredResults != [] && (
          <div className="shadow rounded-md w-full h-full p-2 flex flex-col items-center justify-center sm:grid sm:grid-2 md:grid-cols-2 lg:grid-cols-3">
            {displayCatFilteredResults?.map((cat: any, key: any) => {
              return (
                <div
                  key={key}
          ...
          <div className="flex flex-col items-center md:items-start md:flex-row">
            <div className="flex flex-col py-5 md:py-0">
              ...
              <div className="flex flex-col w-90 shadow bg-zinc-50 p-3 rounded-md border border-purple-200 ">
                ...
                {selectedSpecies === "Dog" ? (
                  <ListButton
                    type={selectedDogBreed}
                    func={dogBreedFilter}
                    data={dogBreeds}
                  />
                ) : (
                  <ListButton
                    type={selectedCatBreed}
                    func={catBreedFilter}
                    data={catBreeds}
                  />
                )}
                ...
                <div className="relative mt-1">
                  {selectedSpecies === "Dog" ? (
                    <ListButton
                      type={selectedDogAge}
                      func={dogAgeFilter}
                      data={["Puppy", "Young", "Adult", "Senior"]}
                    />
                  ) : (
                    <ListButton
                      type={selectedCatAge}
                      func={catAgeFilter}
                      data={["Kitten", "Young", "Adult", "Senior"]}
                    />
                  )}
                </div>
                ...
                <div className="relative mt-1">
                  {selectedSpecies === "Dog" ? (
                    <ListButton
                      type={selectedDogSize}
                      func={dogSizeFilter}
                      data={["Small", "Medium", "Large", "Extra Large"]}
                    />
                  ) : (
                    <ListButton
                      type={selectedCatSize}
                      func={catSizeFilter}
            ...
            <div className="flex flex-col mt-5 md:mt-0 sm:mb-5 sm:ml-5">
              <h3 className="text-2xl md:text-xl text-center sm:text-left">
                Results
              </h3>
